...
@pytest.mark.parametrize("file1, file2, format, expected_output", [
    ...
    (Path("tests/fixtures", "file1.json").resolve(), Path("tests/fixtures", "file2.json").resolve(), "plain", "expected_plain_output"),
    (Path("tests/fixtures", "file1.yaml").resolve(), Path("tests/fixtures", "file2.yaml").resolve(), "plain", "expected_plain_output"),
    (Path("tests/fixtures", "file1.json").resolve(), Path("tests/fixtures", "file2.json").resolve(), "json", "expected_json_output"),
    (Path("tests/fixtures", "file1.yaml").resolve(), Path("tests/fixtures", "file2.yaml").resolve(), "json", "expected_json_output")
])
